  ...
  <div class="createPost-container">
    ...
    <el-form ref="postForm" :rules="rules" class="form-container">
      ...
      <div class="createPost-main-container">
        ...
        <el-form-item
          style="margin-bottom: 0px; max-width: 725px"
          label-width="100px"
          label="이벤트 문장:"
        >
          <el-input
        ...
        <input
          type="number"
          v-model="postForm.appUseFor.totalChanceKeywordChallenge"
        />
        ...
        >
          <el-input
            v-model="postForm.uiConfigOption.shopAppTitle"
            :rows="1"
            type="textarea"
            maxLength="15"
            class="article-textarea"
        ...
        <input
          type="color"
          v-model="postForm.uiConfigOption.shopAppColors.headerColor"
        />
        ...
        <input
          type="color"
          v-model="postForm.uiConfigOption.shopAppColors.mainSpaceColor"
        />
        <span style="line-height: 35px"><br /></span>
        ...
        <input
          type="color"
          v-model="postForm.uiConfigOption.shopAppColors.inputBoxColor"
        />
        <span style="line-height: 35px"><br /></span>
        ...
        <input
          type="color"
          v-model="postForm.uiConfigOption.shopAppColors.userMessageBoxColor"
        />
        <span style="line-height: 35px"><br /></span>
      ...
      <el-button
        ...
        @click="submitChangedAppSettings"
      >
        변경사항 적용
      </el-button>
    </el-form>
...
export default {
  ...
  async mounted() {
    ...
    this.displayIdKeyword =
      this.$store.state.aimedison.keywordShopAimedison[0].keywordId;
    this.displayKeyword =
      this.$store.state.aimedison.keywordShopAimedison[0].value;
  ...
  methods: {
    async getKeywordAppInfo() {
      ...
      this.$store.commit(
        "aimedison/setKeywordShopAimedison",
        resFromKeywordAppServer.data.appUseFor.appEventKeywordList
      );

      this.postForm.uiConfigOption.shopAppColors.answerMessageBoxColor =
        this.convertRgbToHex(
          resFromKeywordAppServer.data.uiConfigOption.shopAppColors
            .answerMessageBoxColor
        );
      this.postForm.uiConfigOption.shopAppColors.headerColor =
        this.convertRgbToHex(
          resFromKeywordAppServer.data.uiConfigOption.shopAppColors.headerColor
        );
      this.postForm.uiConfigOption.shopAppColors.inputBoxColor =
        this.convertRgbToHex(
          resFromKeywordAppServer.data.uiConfigOption.shopAppColors
            .inputBoxColor
        );
      this.postForm.uiConfigOption.shopAppColors.mainSpaceColor =
        this.convertRgbToHex(
          resFromKeywordAppServer.data.uiConfigOption.shopAppColors
            .mainSpaceColor
        );
      this.postForm.uiConfigOption.shopAppColors.userMessageBoxColor =
        this.convertRgbToHex(
          resFromKeywordAppServer.data.uiConfigOption.shopAppColors
            .userMessageBoxColor
        );
      ...
      setTimeout(() => {
        ...
        if (window.document.querySelectorAll(".cell")) {
          if (window.document.querySelectorAll(".cell").length > 0) {
            for (
              let i = 0;
              i < window.document.querySelectorAll(".cell").length;
              i++
            ) {
              window.document.querySelectorAll(".cell")[i].style.paddingLeft =
                "0px";
              window.document.querySelectorAll(".cell")[i].style.paddingRight =
                "0px";
            }
          }
        }
    ...
    async submitChangedAppSettings() {
      this.postForm.appUseFor.appEventKeywordList =
        this.$store.state.aimedison.keywordShopAimedison;

      let hexadecimalHeaderCode =
        this.postForm.uiConfigOption.shopAppColors.headerColor;
      ...
      const keywordAppUseFor = {
        ...
        totalChanceKeywordChallenge:
          this.postForm.appUseFor.totalChanceKeywordChallenge,
      };
    ...
    applyResponsiveStyle() {
      ...
      if (window.innerWidth > 1550) {
        document.getElementById("submitchangebutton").style.marginLeft =
          "1395px";
